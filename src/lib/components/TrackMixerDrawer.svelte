<script lang="ts">
	// Track mixer. A bottom drawer opened from the menubar that gathers every
	// per-track mixing control in one place: a frozen track-list column on the
	// left (name, mute/solo, volume fader, pan knob, EQ) and a horizontally
	// scrollable arrangement timeline on the right, with a Master strip and
	// section markers underneath. All edits apply immediately.
...
<Drawer.Root bind:open direction="bottom">
	<Drawer.Content
		class="flex h-[88vh] max-h-[88vh] flex-col rounded-t-2xl border-t p-0 outline-none"
	>
		<Drawer.Header class="flex-row items-center justify-between gap-2 border-b px-4 py-3">
			<div class="flex items-center gap-2">
				<Sliders class="size-5" />
				<Drawer.Title class="text-base">Tracks</Drawer.Title>
				<Drawer.Description class="sr-only">
					Mix levels, panning and EQ for each track, view the arrangement and manage section
					markers.
				</Drawer.Description>
			</div>
			<div class="flex items-center gap-1.5">
				...
				<Drawer.Close class="text-muted-foreground hover:text-foreground p-1">
					<X class="size-5" />
				</Drawer.Close>
			</div>
		</Drawer.Header>
			...
			<div class="relative w-max min-w-full text-sm">
				...
				{#each tracks as track, i (track.id)}
					...
					<div class={cn('flex border-b', active && 'bg-muted/40')}>
						...
						>
							<div class="flex items-center gap-1.5">
								<span class="size-2.5 shrink-0 rounded-full" style="background:{track.color}"
								></span>
								<input
				...
				<div class="flex">
					...
					>
						...
						{#if sections.length === 0}
							<span class="text-muted-foreground/70 absolute top-3.5 left-2 text-[11px]">
								No sections yet — “Add” drops a marker at the current bar.
							</span>
						{/if}
					</div>
				</div>
